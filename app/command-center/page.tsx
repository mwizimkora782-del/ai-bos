...
import {
  ArrowLeft, Send, Loader2, Mic, Sparkles,
...
const STARTERS = [
  ...
  { icon: <Zap size={15} />,         color: 'text-rose-400',    label: 'Reduce my expenses',       prompt: 'Help me find ways to reduce business expenses without hurting growth.' },
];

// ─── Which AI agent handles what ─────────────────────────
const detectAgent = (text: string): { label: string; color: string } => {
  const t = text.toLowerCase();
  if (t.includes('market') || t.includes('ad') || t.includes('campaign') || t.includes('customer') || t.includes('brand'))
  ...
  if (t.includes('task') || t.includes('plan') || t.includes('week') || t.includes('schedule') || t.includes('goal'))
  ...
  if (t.includes('grow') || t.includes('strateg') || t.includes('scale') || t.includes('expand'))
  ...
  const send = async (text: string) => {
    ...
    try {
      const res  = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: text,
          systemContext: `You are the AI Command Center of AI-BOS — an autonomous business operating system. You are acting as the "${label}". Give powerful, specific, actionable business advice. Be direct, concise, and energetic. Format responses with clear sections when helpful. Always end with 1 concrete next action the business owner should take TODAY.`
        }),
      });
    ...
    <div className="flex flex-col h-screen bg-neutral-950 text-white overflow-hidden">
      ...
      <div className="px-4 pb-6 pt-2 bg-neutral-950 border-t border-neutral-900 flex-shrink-0">
        ...
        <form onSubmit={handleSubmit} className="flex items-end gap-2">
          ...
          <button
            type="submit"
            disabled={loading || paywalled || !input.trim()}
